/* glpapi13.c (routines for reading/writing data in MPS format) */
...
static int read_mps(LPX *lp, const char *fname)
...
static int write_mps(LPX *lp, const char *fname)
{     XFILE *fp;
...
static int read_freemps(LPX *lp, const char *fname)
...
static int write_freemps(LPX *lp, const char *fname)
{     XFILE *fp;
      int wide = lpx_get_int_parm(lp, LPX_K_MPSWIDE);
      int skip = lpx_get_int_parm(lp, LPX_K_MPSSKIP);
      int marker = 0; /* intorg/intend marker count */
